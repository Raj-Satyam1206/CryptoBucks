...
export const CryptoProvider = ({ children }) => {
  ...
  const getCryptoData = async () => {
    const cacheKey = `${currency}_${sortBy}_${page}_${perPage}`;
    if (cache.current.cryptoData[cacheKey]) {
      setCryptoData(cache.current.cryptoData[cacheKey]);
      return;
    }
    ...
    try {
      const response = await fetch(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&order=${sortBy}&per_page=${perPage}&page=${page}&sparkline=false&price_change_percentage=1h%2C24h%2C7d`,
        API_OPTIONS
      );
      ...
      const data = await response.json();
      setCryptoData(data);
      cache.current.cryptoData[cacheKey] = data; // Cache the response
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getCoinData = async (coinid) => {
    if (cache.current.coinData[coinid]) {
      setCoinData(cache.current.coinData[coinid]);
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      setCoinData(data);
      cache.current.coinData[coinid] = data; // Cache the response
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getSearchResult = async (query) => {
    if (cache.current.searchData[query]) {
      setSearchData(cache.current.searchData[query]);
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      setSearchData(data.coins);
      cache.current.searchData[query] = data.coins; // Cache the response
    } catch (error) {
      console.log(error);
    }
  };
